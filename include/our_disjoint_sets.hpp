...
class our_disjoint_sets{
	public:
		//qua tipo dovremmo fare un bel po' di typedefs con tipo Label_map::key_type = lbel_key_type, Label_map::value_type = label_value_type, ecc.
		...
		typedef typename std::map<unsigned int, std::list> Components_map_t;		//Possibilità di considerare anche il numero di elementi nella lista e aggiornarlo man mano per velocizzare inserzioni di pochi vertici in gruppi da tanti vertici
		...
		//! It creates the label map starting form the Graph
		void make_label_map(){
		...
		void new_component(Components_key_t const& label_value){
			components_map[label_value];
		};
		
		//! It creates a new component with the given label value as key in components_map, and assign as value the other parameter
		void new_component_with_insertion(Components_key_t const& label_value, Components_mapped_t vertex){
			components_map[label_value] = vertex;		//what we will insert will be a vertex_descriptor. That's why we named it vertex.
		};
		
		//! It add the given vertex descriptor to the component with that label
		void add_vertex_in_components(Components_key_t const& label_value, Components_mapped_t vertex){
			components_map[label_value] 
		};
