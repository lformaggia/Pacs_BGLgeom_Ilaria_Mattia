...
\section{The library}
	...
	\newline\newline
	\textcolor{red}{Mettere qua eventualmente la parte di codice in più su come aggiungere vertici e edge! E al limite spostare la frase precedente sotto, rimaneggiandola, tipo dicendo che qui facciamo vedere come si fa con le nostre cose, mentre se uno vuole là fa vedere sia come si fa con le nostre cose ma anche le differenze rispetto a come si faceva con la BGL.}
	\newline
	\textcolor{magenta}{Qui sott specifica name=create\_graph se vuoi continuare dall'esempio prima, altrimenti (forse meglio) non mettere niente e comincia con un esempio nuovo, visto che dovrai anche includere graph\_access.hpp e graph\_builder.hpp, e farlo a metà di un codice non avrebbe senso.. Si può mettere anche didascalia come con le figure, ma mi sembra bene integrato nel testo quindi lascerei anche stare.}
	\newline
	\textcolor{green}{Ultima cosa, poi la smetto: se aggiungi cose qui, attenzione che in fondo sbarellano tutte le posizione delle immagini. Buon lavoro :)}
	\begin{lstlisting}[frame=single, name=create_graph]
		G[e].diameter=value;
	\end{lstlisting}
...
In this section we present two examples from real situations, using tools from BGLgeom library to model and solve them.
	...
	\begin{figure}
		\centering 
		\includegraphics[width=0.5\textwidth]{"frac_inters.eps"}
		\caption{Example}
...
\newline\newline
We wants also to thank Dr. Pasquale Africa for its technical support when nothing was compiling on our virtual machine. Always helpul, he gave us a lot of advice on how to write a good Makefile and some suggestions on how to design our code and the library structure.
\newline\newline
Che dici? Ti piace?
